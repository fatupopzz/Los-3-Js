...
const MovieApp = () => {
  ...
  // Función para obtener preferencias del usuario (debug)
  const debugUserPreferences = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log('🔍 Preferencias del usuario:', data);
      
      const interactionsResponse = await fetch(`${API_BASE_URL}/users/${currentUser.id}/interactions`);
      const interactions = await interactionsResponse.json();
      console.log('🔍 Interacciones del usuario:', interactions);
      
      showNotification('Revisa la consola para ver tus preferencias', 'success');
    } catch (error) {
  ...
  const LoginForm = () => {
    ...
    const handleAuth = async () => {
      ...
      try {
        ...
        // Fallback para modo demo - crear usuario único
  ...
  // Componente de tarjeta de película mejorado
        ...
        {movie.recommendation_type && (
          <div className="mb-3">
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${
              ...
              {movie.recommendation_type === 'content_based' ? 'Personalizada' : 'Popular'}
            </span>
          </div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={debugUserPreferences}
              className="text-xs bg-purple-100 text-purple-600 px-2 py-1 rounded-lg hover:bg-purple-200 transition-colors"
              title="Ver preferencias en consola"
            >
              🔍 Debug
            </button>
            <button
